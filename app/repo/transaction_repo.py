...
class TransactionRepository(BaseRepo[TransactionModel]):
    ...
    async def get_report_by_filter(self, query: Select, filters: TransactionFilter) -> Select:
        # Базовый запрос с фильтрацией по дате, статусам, типам
        if filters.start_date:
            start_datetime = datetime.combine(filters.start_date_parsed, datetime.min.time())
            query = query.where(self.trans_model.date_pay >= start_datetime)
        if filters.end_date:
            end_datetime = datetime.combine(filters.end_date_parsed, datetime.max.time())
    ...
    async def get_aggregated_report(self, filters: TransactionFilter) -> AggregateReport:
        ...
        if filters.status == APITypeStatusEnum.SUCCESSFUL:
            if filters.include_avg:
                base_aggr.append(func.avg(self.trans_model.sum_pay).label(TransactionFieldEnum.AVG_AMOUNT.value))
                resul_report_aggr.update({TransactionFieldEnum.AVG_AMOUNT.value: 0})
            if filters.include_max:
                base_aggr.append(func.max(self.trans_model.sum_pay).label(TransactionFieldEnum.MAX_AMOUNT.value))
                resul_report_aggr.update({TransactionFieldEnum.MAX_AMOUNT.value: 0})
            if filters.include_min:
                base_aggr.append(func.min(self.trans_model.sum_pay).label(TransactionFieldEnum.MIN_AMOUNT.value))
                resul_report_aggr.update({TransactionFieldEnum.MIN_AMOUNT.value: 0})
            if filters.include_total:
                base_aggr.append(func.count(self.trans_model.id).label(TransactionFieldEnum.TRANSACTION_COUNT.value))
                resul_report_aggr.update({TransactionFieldEnum.TRANSACTION_COUNT.value: 0})
            if filters.include_avg or filters.include_max or filters.include_min or filters.include_total:
            ...
            for field in resul_report_aggr.keys():
                resul_report_aggr.update({field: float(row._get_by_key_impl_mapping(field))})
    ...
    async def get_daily_shifts(self, filters: TransactionFilter):
        ...
        cte_filtered = await self.get_report_by_filter(
            select(self.trans_model.date_pay, self.trans_model.sum_pay),
            filters
        )
        cte_filtered = cte_filtered.cte(name=TransactionFieldEnum.FILTERED_TRANSACTION.value)
        # Агрегируем по дням
        day_trunc_expr = func.date_trunc(TransactionFieldEnum.DAY.value, cte_filtered.c.date_pay)
        base_query = select(
            day_trunc_expr.label(TransactionFieldEnum.DAY_DATE.value),
            func.sum(cte_filtered.c.sum_pay).label(TransactionFieldEnum.DAILY_TOTAL.value),
            func.count().label(TransactionFieldEnum.DAILY_COUNT.value)
        ).select_from(cte_filtered).group_by(day_trunc_expr)
        ...
        with_prev = select(
            ...
            func.lag(cte_daily_totals.c.daily_total).over(
                order_by=cte_daily_totals.c.day_date
            ).label(TransactionFieldEnum.PREV_DAY_TOTAL.value)
        ).select_from(cte_daily_totals).subquery()
            ...
            case(
                ...
                else_=func.round(
                    ((with_prev.c.daily_total - with_prev.c.prev_day_total) /
                     with_prev.c.prev_day_total) * 100, 2
                )
