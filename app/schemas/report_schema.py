...
class TransactionFilter(BaseModel):
    start_date: Optional[str] = Field(
        default=None, description="Начало периода (формат: YYYY-MM-DD)"
    )
    end_date: Optional[str] = Field(
        default=None, description="Конец периода (формат: YYYY-MM-DD)"
    )
    ...
    def validate_aggregation_fields_with_status(self):
        ...
        has_aggregation_enabled = any(
            getattr(self, field) for field in aggregation_fields
        )
        if has_aggregation_enabled and self.status != APITypeStatusEnum.SUCCESSFUL:
            enabled_fields = [
                field for field in aggregation_fields if getattr(self, field)
            ]
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Поля агрегации {enabled_fields} могут быть включены только при status={APITypeStatusEnum.SUCCESSFUL}",
            )
        return self
...
class CountryStatsFilter(BaseModel):
    sort_by: Literal["count", "total", "avg"] = Field(
        default="count", description="Метрика для сортировки: count, total, avg"
    )
    top_n: Optional[int] = Field(
        default=None, description="Ограничение количества стран", ge=1
    )
